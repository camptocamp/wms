...
def setup_stock_picking_note__picking(env):
    """Create a new picking note type named 'picking'."""

    _logger.info("Create a new picking note type named 'picking'")
    env["stock.picking.note.type"].create({"name": "picking"})
...
def setup_stock_picking_note__packing(env):
    ...
    env["stock.picking.note.type"].create({"name": "packing"})
...
def populate_stock_picking_note__packing(env):
    ...
    for partner in partners:
        note = env["stock.picking.note"].search(
            [("name", "=", partner.shopfloor_packing_info_id.text)]
        )
        if note:
            partner.stock_picking_note_ids = partner.stock_picking_note_ids | note
...
def migrate(cr, version):
    ...
    setup_stock_picking_note__picking(env)
    setup_stock_picking_note__packing(env)
