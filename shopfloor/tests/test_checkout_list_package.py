...
class CheckoutScanSetDestPackageCase(CheckoutCommonCase, SelectDestPackageMixin):
    ...
    def _get_allowed_packages(self):
        return (
            self.picking.mapped("move_line_ids.package_id")
            | self.picking.mapped("move_line_ids.result_package_id")
    ...
    def test_set_dest_package_error_not_found(self):
        ...
        self._assert_response_select_dest_package(
            ...
            self.selected_lines,
            self._get_allowed_packages(),
            # This case is not supposed to happen with 'set_dest_package' as
            # we pass an existing package ID as parameter, so we don't really
            # care about the error message mentionning a barcode 'None'
            message=self.service.msg_store.package_not_found_for_barcode(None),
        )
