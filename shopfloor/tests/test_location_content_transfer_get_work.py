...
class TestLocationContentTransferGetWork(LocationContentTransferCommonCase):
    ...
    * /cancel_work
    ...
    def setUpClassBaseData(cls):
        ...
        # Make the picking 2 the oldest one, so the next one to work on
        cls.picking2.create_date = "2000-01-01 12:00:00"
        cls.next_location = cls.picking2.move_line_ids[0].location_id
    ...
    def test_find_work_work_found(self):
        ...
        self.assert_response(
            ...
            data={
                "location": self.data.location(self.next_location),
            },
        )
        lines = self._get_location_lines(self.next_location)
        self.assertEqual(lines.shopfloor_user_id, self.env.user)

    def test_cancel_work(self):
        stock = self.service._actions_for("stock")
        location_lines = self._get_location_lines(self.next_location)
        stock.mark_move_line_as_picked(location_lines)
        location_lines = self._get_location_lines(self.next_location)
        self.assertEqual(location_lines.shopfloor_user_id, self.env.user)
        response = self.service.dispatch(
            "cancel_work", params={"location_id": self.next_location.id}
        )
        self.assert_response(
            response,
            next_state="get_work",
