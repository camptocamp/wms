...
class Checkout(Component):
    ...
    def list_delivery_packaging(self, picking_id, selected_line_ids):
        """List available delivery packaging based on the carrier of the
        picking to create the new package.
    ...
    def _set_dest_package_from_selection(
        self, picking, selected_lines, package, barcode=None
    ):
        if not package:
    ...
    def scan_dest_package(self, picking_id, selected_line_ids, barcode):
        ...
        * summary: all lines are put in packages
        ...
        search = self._actions_for("search")
        package = search.package_from_scan(barcode)
        return self._set_dest_package_from_selection(picking, lines, package, barcode)
