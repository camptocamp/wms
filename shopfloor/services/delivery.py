...
class Delivery(Component):
    ...
    * Bin packed products are placed in new shipping package and shipped to customer
    ...
    def scan_deliver(self, barcode, picking_id=None, location_id=None):
        ...
        When a stock location is scanned, and it is a sublocation of an operation
        type set on the shopfloor current shopfloor menu. The next delivery
        operations will be localized to that location.
        ...
        * deliver: always return here with the data for the last touched
        ...
        if not barcode_valid:
            ...
            if product:
                return self._deliver_product(location, picking, product)
        ...
        if not barcode_valid and allow_prepackaged_product:
            ...
            if packaging:
                ...
                return self._deliver_product(
                    location, picking, packaging.product_id, product_qty=packaging_qty
                )

        if not barcode_valid:
            sublocation = search.location_from_scan(barcode)
            if sublocation and sublocation.is_sublocation_of(
                self._get_process_picking_types().mapped("default_location_src_id")
            ):
    ...
    def _lines_from_product_domain(
        self, location, product, no_qty_done=True, product_qty=None
    ):
    ...
    def _deliver_product(self, location, picking, product, product_qty=None):
        ...
        lines = self.env["stock.move.line"].search(
            self._lines_from_product_domain(location, product, product_qty=product_qty),
            limit=limit,
            order="date_planned",
        )
        if not lines:
        ...
        if multiple_location:
            # Asking to scan a specific location, but we are going to loose the barcode
            return self._response_for_deliver(
                picking,
                location=location,
