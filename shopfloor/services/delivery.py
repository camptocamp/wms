...
class Delivery(Component):
    ...
    * Bin packed products are placed in new shipping package and shipped to customer
        ...
        if not barcode_valid and allow_prepackaged_product:
            ...
            if packaging:
                ...
                return self._deliver_product(
                    picking, packaging.product_id, packaging_qty
                )
    ...
    def _set_lines_done(self, lines, packaging_qty=None):
        ...
        if packaging_qty:
            ...
            packaging_move_qty = lines.move_id.product_id.uom_id._compute_quantity(
                packaging_qty, lines.move_id.product_uom
            )
            lines.qty_done = packaging_move_qty
    ...
    def _lines_from_product_domain(self, product, no_qty_done=True, packaging_qty=None):
    ...
    def _deliver_product(self, picking, product, packaging_qty=None):
        if product.tracking in ("lot", "serial"):
            return self._response_for_deliver(
                picking, message=self.msg_store.scan_lot_on_product_tracked_by_lot()
            )

        limit = 1 if packaging_qty else None
        lines = self.env["stock.move.line"].search(
            self._lines_from_product_domain(product, packaging_qty=packaging_qty),
            limit=limit,
        )
        if not lines:
            return self._response_for_deliver(
        ...
        if self._set_lines_done(lines, packaging_qty):
            return self._response_for_deliver(
                message=self.msg_store.transfer_complete(new_picking)
            )
