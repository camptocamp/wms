    ...
    def _package_parser(self, picking=None):
        ...
        if picking:
            parser.append(
                (
                    "pack_weight:weight",
                    lambda rec, fname, picking=picking: rec.pack_weight
                    or rec.with_context(picking_id=picking.id).estimated_pack_weight,
                ),
            )
        else:
            parser.append(
                (
                    "pack_weight:weight",
                    lambda rec, fname: rec.pack_weight or rec.estimated_pack_weight,
                )
            )
        return parser
