...
class StockMove(models.Model):
    ...
    def _apply_routing_rule_pull(self, routing_details):
        ...
        for move in self:
            ...
            # pull routing rules
            original_location = move.location_id
            ...
            if move.location_id == original_location:
                move_to_assign_ids.append(move.id)
            else:
                move_to_assign_first_ids.append(move.id)
        ...
        #
        # Now among these moves, some may have their source location changed,
        # for instance:
        #
        ...
        #   * 10 in Stock/Reserve
        # * Stock/Reserve insert a new move to do Stock/Reserve -> Stock/Shelving
        #
        # The source location for the reserve move is now Stock/Reserve.
        # If the move to reserve the first move in Stock/Shelving is reserved
        # first, it may take the goods in Stock/Reserve, which should be
        # reserved by the second one.
        #
        # That's why, if the source location has changed, we reserve it first.
        # We assume that if it has changed, it is either more precise, either
        # in a different location tree, so it shouldn't steal goods of other
        # moves.
        #
        ...
        to_assign_ids = move_to_assign_first_ids + move_to_assign_ids
