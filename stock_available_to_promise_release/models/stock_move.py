...
class StockMove(models.Model):
    ...
    def _unrelease_set_returneable_qty_per_move(
        ...
        for move in self:
            ...
            # Get it, and deduce it from the returneable qty
            ...
            move_returneable_qty = min(
                qty_to_return, move.product_qty - move_returned_qty - move_qty_planned
            )
            if float_is_zero(move_returneable_qty, precision_rounding=rounding):
                continue
            # Update the quantity
            qty_to_return_per_move[move.id] += move_returneable_qty
            qty_to_return -= move_returneable_qty
            returneable_qty += move_returneable_qty
            if float_is_zero(qty_to_return, precision_rounding=rounding):
                break
        return returneable_qty
    ...
    def unrelease(self, safe_unrelease=False):
        ...
        for move in moves_to_unrelease:
            ...
            for origin_moves in iterator:
                ...
                returneable_qty = done_moves._unrelease_set_returneable_qty_per_move(
                    qty_to_return, qty_to_return_per_move
                )
                qty_returned_for_move += returneable_qty
